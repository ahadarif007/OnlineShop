...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
		...
		if (empty($_POST["phone"]) || strlen($_POST["phone"])<11) 
		 ...
		 {
			echo"<p style='color:red'>shipping Address is required </p><br/>";
			$v9=0;
		 } 
		 else 
		 {
...
$password=md5($pass);
$sql="INSERT INTO userdata (firstname, lastname, dateOfBirth, gender, phone, email, shippingAddress, password, Picture) 
VALUES('".$firstname."', '".$lastname."', '".$dateOfBirth."', '".$gender."', '".$phone."', '".$email."', '".$shippingAddress."', '".$password."', '".$Picture."')";

	//print_r($sql);
	$result=insert($sql);
    if($result=true)
    {
		echo "<hr><h1>Account Created Successfully  </h1> <hr>";
		?> 
		<br>
		<a href="../userLogin/userLogin.html"> Sign In</a>
		<?php
    }
