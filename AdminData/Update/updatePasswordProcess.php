...
if($_SESSION["isValid"]=="yes" && isset($_REQUEST["cpass1"]) && isset($_REQUEST["cpass2"]) && isset($_REQUEST["pass"]))
{
    ...
    $data=array();
    include("../lib.php");
	$sql="select * from admindata where email ='".$_SESSION["id"]."' AND password ='".md5($_REQUEST["pass"])."' ";
	//print_r($sql);
    loadFromMySQL($sql);
    echo "<h1>";
    //$_session("id");
    if(sizeof($data)>0)
    {
        if($_REQUEST["cpass1"]==$_REQUEST["cpass2"])
        {
            ...
            $sql="UPDATE admindata SET password='".md5($_REQUEST["cpass1"])."' WHERE email='".$_SESSION["id"]."'";
	    //print_r($sql);
	    $result=update($sql);
             if($result=true)
             {
...
session_destroy();
header("Location:\projectFinal\index.php");
